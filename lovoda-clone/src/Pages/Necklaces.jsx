...
const Necklaces = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`https://lovodaapi.onrender.com/necklace?_page=${page}&_limit=16&_sort=${sort}&_order=${order}&price_gte=${greater}&price_lte=${less}`)
            .then((res) => res.json())
            .then((res)=>{
                setData(res) ;
                setLoading(false);  
    ...
    return (
        ...
        <Box className="main">
            ...
            <Flex my="1.5rem"  fontSize={"sm"} >
                ...
                <Flex w="10%" mt="5px" ml="20px" fontSize="large">364 products</Flex>
            </Flex>
            {/* Products */}
        ...
        <Box w={"90%"} m="auto" mt={"2rem"} mb={5}>
            <Heading textAlign={"left"} fontWeight={400} size={"lg"}>Necklaces</Heading>
            {/* Sorting Starts */}
            <Flex my="1.5rem"  fontSize={"sm"} >
                <Flex  w="50%" alignItems={"center"} ml="1rem">
                    <Text ml="-13px" mr="1rem">Filter :</Text>
